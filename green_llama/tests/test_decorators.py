...
class TestMetricsDecorators(unittest.TestCase):
    ...
    def test_measure_all_metrics(self, mock_emissions_tracker, mock_energy_tracker):
        ...
        self.assertEqual(metrics["CPU Energy (J)"], 10.0)
        self.assertEqual(metrics["GPU Energy (J)"], 20.0)
        self.assertEqual(metrics["RAM Energy (J)"], 5.0)
        self.assertEqual(metrics["Total Energy (J)"], 35.0)
        self.assertEqual(metrics["Carbon Emissions (gCO2)"], 8.0)
